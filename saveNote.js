saveNotes = () => {
  ...
  saveNoteData = localStorage.setItem(
    "saveNotes",
    JSON.stringify(saveNoteData)
  );
};
...
loadNote = () => {
  ...
  for (let i = 0; i < notes.length; i++) {
    ...
    iframe[0].addEventListener("load", () => {
      iframe[0].contentWindow.document.getElementById("header").innerHTML =
        saveNoteData[i].header;
      iframe[0].contentWindow.document.getElementById("content").innerHTML =
        saveNoteData[i].content;
    });
  }
};
